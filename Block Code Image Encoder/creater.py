...
class image_handler(block_code):
    '''
    Works only with files .jpg and .json

    Instance creation syntax:
    a = image_handler('path')

    Methods:
    a.image_with_error(N) - splits the image into pixels in RGB format and allows N errors in each color.

    a.image_to_json() - splits the image into pixels in RGB format, 
    converts each color into a code word with an error using block code and writes the result to a JSON file.

    a.image_to_jpg() - takes a list of code words from a JSON file, corrects errors in them, 
    converts them to RGB format and draws a decoded image.
    '''
    ...
    def image_with_errors(self, quantity):
        if quantity > 8 or quantity < 1:
            raise TypeError('0 < number of errors < 9')
...
if __name__ == '__main__':
    new_file = image_handler('C:/Users/toki0/OneDrive/Документы/дгшка/TI/lab3/photos/smile.jpg')
